...
`,
	...
	cmdEthTransfer.Flags().Var(&fromAddress, "from", "sender's woc address, keystore must exist locally")
	cmdEthTransfer.Flags().Var(&toAddress, "to", "the destination woc address")
	...
	cmdEthTransfer.Flags().StringVar(&amount, "amount", "0", "amount to send (WOC)")
