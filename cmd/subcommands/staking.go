...
var (
	...
	blsPubKeyDir              string
	delegatorAddress          wocAddress
	validatorAddress          wocAddress
...
var (
	...
	errMinSelfDelegationTooSmall       = errors.New("min-self-delegation can not be less than 1 WOC")
	errMaxTotalDelegationTooSmall      = errors.New("max-total-delegation can not be less than 1 WOC")
...
func handleStakingTransaction(
	stakingTx *staking.StakingTransaction, networkHandler *rpc.HTTPMessenger, signerAddress wocAddress,
) error {
	var (
...
func delegationAmountSanityCheck(minSelfDelegation *numeric.Dec, maxTotalDelegation *numeric.Dec, amount *numeric.Dec) error {
	// MinSelfDelegation must be >= 1 WOC
	if minSelfDelegation != nil && minSelfDelegation.LT(wocAsDec) {
		return errMinSelfDelegationTooSmall
	}

	// MaxTotalDelegation must be a
	if maxTotalDelegation != nil && maxTotalDelegation.LT(wocAsDec) {
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			amt = amt.Mul(wocAsDec)
			minSelfDel = minSelfDel.Mul(wocAsDec)
			maxTotalDel = maxTotalDel.Mul(wocAsDec)
