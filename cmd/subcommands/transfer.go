...
var (
	fromAddress       wocAddress
	toAddress         wocAddress
	amount            string
	fromShardID       uint32
	toShardID         uint32
	targetChain       string
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if givenFilePath == "" {
				...
				if hasError {
					return fmt.Errorf("woc or more of your transactions returned an error " +
	...
	cmdTransfer.Flags().Var(&fromAddress, "from", "sender's woc address, keystore must exist locally")
	cmdTransfer.Flags().Var(&toAddress, "to", "the destination woc address")
	...
	cmdTransfer.Flags().StringVar(&amount, "amount", "0", "amount to send (WOC)")
...
`,
	...
	cmdGetNonce.Flags().Var(&fromAddress, "from", "sender's woc address, keystore must exist locally")
