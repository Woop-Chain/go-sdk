...
func validateAddress(cmd *cobra.Command, args []string) error {
	// Check if input valid woc address
	tmpAddr := wocAddress{}
	if err := tmpAddr.Set(args[0]); err != nil {
		...
		if bech32Addr == "" {
			return fmt.Errorf("Invalid woc address/Invalid account name: %s", args[0])
		}

		tmpAddr = wocAddress{bech32Addr}
	}
	addr = tmpAddr
	return nil
}
