...
This is a go layer on top of the WoopChain RPC, included is a CLI tool that you can build with a
...
https://github.com/woop-chain/woop/blob/main/README.md.
...
./wiki --node=https://rpc.woopchain.com balances <SOME_WOC_ADDRESS>

2.  Check sent transaction
./wiki --node=https://rpc.woopchain.com blockchain transaction-by-hash <SOME_TX_HASH>

3.  List local account keys
./wiki keys list

4.  Sending a transaction (waits 40 seconds for transaction confirmation)
./wiki --node=https://rpc.woopchain.com transfer \
    --from <SOME_WOC_ADDRESS> --to <SOME_WOC_ADDRESS> \
...
./wiki --node=https://rpc.woopchain.com transfer --file <PATH_TO_JSON_FILE>
Check README for details on json file format.

6.  Check a completed transaction receipt
./wiki --node=https://rpc.woopchain.com blockchain transaction-receipt <SOME_TX_HASH>
...
./wiki --node=https://rpc.woopchain.com staking create-validator --amount 10 --validator-addr <SOME_WOC_ADDRESS> \
...
./wiki --node=https://rpc.woopchain.com staking edit-validator \
    --validator-addr <SOME_WOC_ADDRESS> --identity foo --details bar \
...
./wiki --node=https://rpc.woopchain.com staking delegate \
    --delegator-addr <SOME_WOC_ADDRESS> --validator-addr <VALIDATOR_WOC_ADDRESS> \
    --amount 10 --passphrase

15. Undelegate to a validator
./wiki --node=https://rpc.woopchain.com staking undelegate \
    --delegator-addr <SOME_WOC_ADDRESS> --validator-addr <VALIDATOR_WOC_ADDRESS> \
    --amount 10 --passphrase

16. Collect block rewards as a delegator
./wiki --node=https://rpc.woopchain.com staking collect-rewards \
    --delegator-addr <SOME_WOC_ADDRESS> --passphrase

17. Check elected validators
./wiki --node=https://rpc.woopchain.com blockchain validator elected

18. Get current staking utility metrics
./wiki --node=https://rpc.woopchain.com blockchain utility-metrics

19. Check in-memory record of failed staking transactions
./wiki --node=https://rpc.woopchain.com failures staking

20. Check which shard your BLS public key would be assigned to as a validator
./wiki --node=https://rpc.woopchain.com utility shard-for-bls <BLS_PUBLIC_KEY>
...
Example:
```
wiki --node="rpc1.woopchain.com/" transfer --file ./batchTransactions.json
```
...
| `to`                | string     | [**Required**] The receivers woc address. |
| `amount`            | string     | [**Required**] The amount to send in $WOC. |
...
| `gas-price`         | string     | [*Optional*] The gas price to pay in NANO (1e-9 of $WOC), default is 1. |
...
[
  {
    "from": "woc103q7qe5t2505lypvltkqtddaef5tzfxwsse4z7",
    "to": "woc1zksj3evekayy90xt4psrz8h6j2v3hla4qwz4ur",
    "from-shard" : "0",
    "to-shard": "0",
    "amount": "1",
    "passphrase-string": "",
...
[
  ...
  },
  {
    ...
    "time-signed-utc": "2024-01-22 22:01:11.468407"
  }
]
```
...
```bash
./wiki get-nonce --node=https://rpc.woopchain.com --from=[WOC address]
```

2. Sign transfer and write to file. (Passphrase required, But no need to be online)
```bash
./wiki transfer --offline-sign --nonce=[nonce value from previous] --from=[WOC address] --to=[WOC address] --amount=1000 --from-shard=0 --to-shard=0 > signed.json
```

3. send `signed.json` to Woop blockchain! (Need to be online, but no passphrase required)
```bash
./wiki offline-sign-transfer --node=https://trpc.woopchain.com --file ./signed.json
```
...
```bash
./wiki command --node="https://trpc.woopchain.com" --net=testnet
```
...
var account = 'woc1xjanr7lgulc0fqyc8dmfp6jfwuje2d94xfnzyd' // change this to your account address, support eth or woc address
