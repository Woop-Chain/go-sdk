...
func ImportFromPrivateKey(privateKey, name, passphrase string) (string, error) {
	...
	wocAddress := address.ToBech32(crypto.PubkeyToAddress(sk.PublicKey))

	if store.FromAddress(wocAddress) != nil {
		return "", fmt.Errorf("address %s already exists", wocAddress)
	}
